...
self.addEventListener("fetch", (event) => {
  event.respondWith(
    (async () => {
      // Priority 1: UV service worker routing for '/a/' URLs
      if (event.request.url.startsWith(`${location.origin}/a/`)) {
        return await uv.fetch(event);
      }

      // Priority 2: Dynamic routing for '/a/q/' URLs
      if (event.request.url.startsWith(`${location.origin}/a/q/`) && await dynamic.route(event)) {
